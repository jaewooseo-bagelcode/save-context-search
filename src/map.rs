...
pub fn format_rust_map(project_name: &str, modules: &[Module], max_tokens: usize) -> String {
    ...
    // Header
    let header = format!(
        "//! 🗺️ Project Map: {}\n//!\n//! WHAT: Public API overview - modules, classes, functions with doc summaries\n//! WHY:  Understand project structure before diving into implementation\n//! HOW:  Find relevant modules here, then use `scs lookup <name>` for details\n\n",
        project_name
    );
